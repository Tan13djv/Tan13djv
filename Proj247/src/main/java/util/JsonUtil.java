...
public class JsonUtil {
    ...
    public static String ConvertStatArrTo(ArrayList<Statistics> ArrStat){
        Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting().create();
        String gsonStr = gson.toJson(ArrStat);
        return gsonStr;
    };
    ...
    public static byte[] SerialToJson(DataStructure struct){

        Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting().create();
        String gsonStr = gson.toJson(struct);
        return gsonStr.getBytes();
    }
}
